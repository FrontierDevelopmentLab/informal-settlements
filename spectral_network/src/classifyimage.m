function classifyimage(CCF,country,source,city,testcity,type,method, ntrees, server)
...
%         disp(['Creating mask prediction of column ' num2str(ii)])
    ...
    dirname = strcat(server,'predictions','/',method, '/',country,'/',city,'/images');
    mkdir_if_not_exist(dirname);
    filename = strcat(dirname,'/',fname);
